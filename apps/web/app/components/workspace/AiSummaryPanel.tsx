...
export function AiSummaryPanel({ roomId, channelId }: { roomId: string; channelId: string }) {
  const [summary, setSummary] = useState<string | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [updatedAt, setUpdatedAt] = useState<string | null>(null); const [cached, setCached] = useState(false);
  async function generate() { setLoading(true); setError(null); try { const result = await getRoomSummary(roomId, channelId); setSummary(result.summary.trim() || null); setUpdatedAt(result.updatedAt); setCached(result.cached); } catch (e) { setError(getApiErrorMessage(e, "Unable to generate a summary.")); } finally { setLoading(false); } }
  useEffect(() => { setSummary(null); setError(null); setUpdatedAt(null); setCached(false); }, [roomId, channelId]);
  return <div className="h-full overflow-y-auto bg-slate-50 px-4 py-8 sm:px-6"><div className="mx-auto flex max-w-2xl flex-col items-center text-center"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Sparkles className="h-6 w-6" /></span><h2 className="mt-3 text-lg font-semibold text-slate-100">AI study-room summary</h2><p className="mt-1 max-w-md text-sm text-slate-500">Concise notes based only on the last 24 hours of this channel&apos;s discussions and materials.</p>{loading ? <div className="mt-8 flex flex-col items-center gap-3 rounded-xl border border-slate-200 bg-white px-8 py-7 text-sm text-slate-600"><Loader2 className="h-6 w-6 animate-spin text-blue-600" />Reviewing recent channel activity…</div> : summary ? <div className="mt-6 w-full"><MarkdownNotes markdown={summary} /><p className="mt-3 text-xs text-slate-500">{cached ? "Cached summary" : "Fresh summary"}{updatedAt ? ` · Updated ${new Date(updatedAt).toLocaleString()}` : ""}</p></div> : error ? <div className="mt-8 max-w-md rounded-xl border border-red-200 bg-red-50 p-5 text-left"><div className="flex gap-2 text-sm font-medium text-red-800"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />Couldn&apos;t generate the summary</div><p className="mt-2 text-sm leading-6 text-red-700">{error}</p></div> : <div className="mt-8 rounded-xl border border-dashed border-slate-300 bg-white px-8 py-7 text-sm text-slate-500">No summary generated yet.</div>}<button onClick={generate} disabled={loading} className="mt-6 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{loading && <Loader2 className="h-4 w-4 animate-spin" />}{summary ? "Refresh summary" : "Generate summary"}</button></div></div>;
}
